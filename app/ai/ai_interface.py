        ...
        with st.spinner(
            ...
            try:
                ...
                if response_text and not response_text.startswith("❌"):
                    # --- FILTRARE AVANSATĂ CORECTATĂ (ELIMINARE BLOCURI THINK) ---
                    # Șterge tot conținutul dintre <think> și </think> indiferent de litere mari/mici
                    response_text = re.sub(r'<think>.*?</think>', '', response_text, flags=re.DOTALL | re.IGNORECASE)
                    # Elimină etichetele izolate rămase în text din cauza limitărilor de context
                    response_text = re.sub(r'</?think>', '', response_text, flags=re.IGNORECASE)

                    st.session_state[session_key] = response_text.strip()
                else:
                    st.warning(
                ...
                st.error(
                    ...
                    "Asigură-te că valoarea GROQ_API_KEY este definită corect în setările platformei."
                )
